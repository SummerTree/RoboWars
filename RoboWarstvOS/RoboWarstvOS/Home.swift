...
class Home: SKScene, GKGameCenterControllerDelegate, SKProductsRequestDelegate, SKPaymentTransactionObserver {
    ...
    override func didMove(to view: SKView) {
        ...
        tapDown.allowedPressTypes = [NSNumber (value:  UIPressType.downArrow.rawValue)]
        self.view!.addGestureRecognizer(tapDown)
        
        tapUp.addTarget(self, action: #selector(Home.selectUp))
        tapUp.allowedPressTypes = [NSNumber (value:  UIPressType.upArrow.rawValue)]
        self.view!.addGestureRecognizer(tapUp)
        
        tapLeft.addTarget(self, action: #selector(Home.selectLeft))
        tapLeft.allowedPressTypes = [NSNumber (value:  UIPressType.leftArrow.rawValue)]
        self.view!.addGestureRecognizer(tapLeft)
        
        tapRight.addTarget(self, action: #selector(Home.selectRight))
        tapRight.allowedPressTypes = [NSNumber (value:  UIPressType.rightArrow.rawValue)]
    ...
    func saveHighScore(_ identifier:String, score:Int) {
        ...
        if ( allowScorePosting == true) {
            ...
            if ( highScore > defaults.integer( forKey: "GameCenterHighScore")) {
            ...
            if let someButton:Button = node as? Button {
                ...
                if (someButton.requiresProduct != "") {
                    ...
                    if ( self.defaults.object(forKey: someButton.requiresProduct) ==  nil){
                        ...
                        print("setting up purchasing for a locked button")
